...
static KRPCFlightReplay *flightReplay;
#endif  // HAS_DEV(XXX_DESKTOP_FILE)
...
#if HAS_DEV(PYRO_SIM_KRPC)
static KRPCPyroCtrl_s pyroKRPC[NUM_PYRO_SIM_KRPC];
#endif  // HAS_DEV(PYRO_SIM_KRPC)
...
static void callbackMisdirectionFun(int a, bool b) {
  flightReplay->writeActionGroup(a, b);
}
...
void hm_hardwareInit() {
  ...
  flightReplay = new KRPCFlightReplay();
...
#if HAS_DEV(PYRO_DESKTOP_PRINT)
  if (IS_DEVICE(pyroId, PYRO_DESKTOP_PRINT)) {
    krpcPyro_set(&printPyro[pyroId - FIRST_ID_PYRO_DESKTOP_PRINT], enable);
  }
#endif  // HAS_DEV(PYRO_DESKTOP_PRINT)
}
...
#if HAS_DEV(PYRO_DESKTOP_PRINT)
  if (IS_DEVICE(pyroId, PYRO_DESKTOP_PRINT)) {
    krpcPyro_pwmStart(&printPyro[pyroId - FIRST_ID_PYRO_DESKTOP_PRINT],
