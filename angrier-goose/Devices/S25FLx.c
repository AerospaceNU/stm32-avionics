...
bool S25FLX_is_connected(S25FLXCtrl_t *s25flx) {
	cs_pull(s25flx, GPIO_PIN_RESET);
	uint8_t cmd[4] = {0x90, 0x0, 0x0, 0x0};
	if (HAL_SPI_Transmit(s25flx->hspi, cmd, 4, SPI_TX_RX_TIMEOUT_MS) != HAL_OK) {
		cs_pull(s25flx, GPIO_PIN_SET);
		return false;
	}
	// Read into given buffer pData
	uint8_t txBuf2[2] = {0, 0};
	uint8_t rxBuf2[2] = {0, 0};
	if (HAL_SPI_TransmitReceive(s25flx->hspi, txBuf2, rxBuf2, 2, SPI_TX_RX_TIMEOUT_MS) != HAL_OK) {
		cs_pull(s25flx, GPIO_PIN_SET);
		return false;
	}

	cs_pull(s25flx, GPIO_PIN_SET);
	return rxBuf2[0] == 0x01 && rxBuf2[1] == 0x17;
}
