...
bool servoInit(ServoCtrl_t *servo, TIM_HandleTypeDef *htim, uint32_t channel, uint32_t periodMS, float minPulseMS, float maxPulseMS, float minAngle, float maxAngle) {
	...
	// Set up timer prescaler for fastest counting possible that can still count through one period of (periodMS * 2 safety factor)
	htim->Init.Prescaler = (uint32_t) ((SystemCoreClock / 1000 * periodMS / 65535) - 1); // * 2 and / 2 cancel out

	// Period should be half the max count of the clock, given how the prescaler was set up
...
void servoSetAngle(ServoCtrl_t *servo, float degrees) {
	...
	float pulsePercent = (degrees - servo->minAngle) / (servo->maxAngle - servo->minAngle);
	float pulseLengthMS = (servo->maxPulseMS - servo->minPulseMS) * pulsePercent + servo->minPulseMS;
