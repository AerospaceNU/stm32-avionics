...
void buzzerInit(BuzzerCtrl_t *buzzer, TIM_HandleTypeDef *htim, uint32_t channel, float minFrequency) {
	...
	// Set up timer prescaler for fastest counting possible that can still produce a signal at (minFrequency / 2)
	// Division by 2 is a safety factor, with the tradeoff being inability to produce very high frequency signals
	buzzer->htim->Init.Prescaler = (uint32_t) ((SystemCoreClock / minFrequency / 65535) - 1); // * 2 and / 2 cancel out

	// Calculation of actually max frequency needed because prescaler could be rounded a lot, resulting in different number than input
	buzzer->minFrequency = SystemCoreClock / ((htim->Init.Prescaler + 1) * 65535 * 2);
}
...
void buzzerSetFrequency(BuzzerCtrl_t *buzzer, float fHz) {
	...
	// Set timer pulse
	__HAL_TIM_SET_COMPARE(buzzer->htim, buzzer->channel, buzzer->htim->Init.Period / 2);
}
