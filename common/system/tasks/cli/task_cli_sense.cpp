...
void CliTasks::sense() {
  ...
  for (int i = 0; i < NUM_GPS; i++) {
    dtoa(float1, sizeof(float1), data->gpsData[i].generalData.latitude, 5);
    dtoa(float2, sizeof(float2), data->gpsData[i].generalData.longitude, 5);
