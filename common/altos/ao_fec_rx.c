...
static const uint8_t ao_interleave_order[] = {
    0x1e, 0x16, 0x0e, 0x06, 0x1c, 0x14, 0x0c, 0x04,
    0x1a, 0x12, 0x0a, 0x02, 0x18, 0x10, 0x08, 0x00};
...
#define DO_STATE(state)                                                     \
  {                                                                         \
    ...
    while ((b - o) >= (8 + NUM_HIST) || (i + 2 >= len && b > o)) {
      ...
      *out++ = byte;
      if (out_len > 2) crc = ao_fec_crc_byte(byte, crc);

      if (!--out_len) {
        if ((out[-2] == (uint8_t)(crc >> 8)) && out[-1] == (uint8_t)crc)
