 ...
 */
static uint8_t ao_fec_prepare(const uint8_t *in, uint8_t len, uint8_t *extra) {
  ...
  num_fec = 2 - (i & 1);
  while (num_fec--) extra[i++] = AO_FEC_TRELLIS_TERMINATOR;
  return i;
}
...
uint8_t ao_fec_encode(const uint8_t *in, uint8_t len, uint8_t *out) {
  ...
  for (pair = 0; pair < len + extra_len; pair += 2) {
    ...
    for (bit = 0; bit < 4 * 4; bit++) {
      ...
      interleave =
          (interleave << 2) | ((encode >> (byte_shift + bit_shift)) & 0x3);
    }
    *out++ = interleave >> 24;
    *out++ = interleave >> 16;
    *out++ = interleave >> 8;
