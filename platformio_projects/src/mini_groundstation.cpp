...
SPISettings spiSettings(1000000, SPI_MSBFIRST, SPI_MODE0);

class ArduinoSx126x : public SX126x {
  uint8_t HalGpioRead(GpioPinFunction_t func) {
    switch (func) {
      case GPIO_PIN_RESET:
        return digitalRead(SX1262_RST);
      case GPIO_PIN_BUSY:
        return digitalRead(SX1262_BUSY);
      case GPIO_PIN_DIO1:
        return digitalRead(SX1262_DIO1);
      case GPIO_PIN_DIO3:
        return digitalRead(SX1262_DIO1);
      default:
        return 0;
  ...
  void HalGpioWrite(GpioPinFunction_t func, uint8_t value) {
    switch (func) {
      case GPIO_PIN_RESET:
        digitalWrite(SX1262_RST, value);
        break;
      case GPIO_PIN_BUSY:
        digitalWrite(SX1262_BUSY, value);
        break;
      case GPIO_PIN_DIO1:
        digitalWrite(SX1262_DIO1, value);
        break;
      case GPIO_PIN_DIO3:
        digitalWrite(SX1262_DIO1, value);
        break;
...
void loop() {
  ...
  Serial.print("Errors on configuration: ");
  Serial.println(radio.GetDeviceErrors().Value);
