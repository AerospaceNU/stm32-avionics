...
void HM_HardwareInit() {
  ...
  /* ICM20948 IMU 2 */
  icm20948.spiconfig.hspi = IMU2_AG_HSPI;
  ...
  ICM_20948_GYRO_CONFIG_t icm20948GyroConfig = {
      .GYRO_FS_SEL = 3, .GYRO_FCHOICE = 1, .GYRO_DLPFCFG = 0, .reserved_0 = 0};
  hardwareStatus[IMU2] =
      ICM20948_init(&icm20948, icm20948AccelConfig, icm20948GyroConfig);
#endif /* FCB_VERSION */
  ...
  /* Checking if inits are successful (inits that don't return a boolean are
   * assumed true) */
...
bool HM_FlashEraseChipStart() { return S25FLX_erase_chip_start(&s25flx1); }
...
bool HM_FlashIsWriteComplete() { return S25FLX_is_write_complete(&s25flx1); }

bool HM_FlashIsEraseComplete() { return S25FLX_is_erase_complete(&s25flx1); }

void HM_BuzzerSetFrequency(float fHz) { buzzerSetFrequency(&buzzer, fHz); }

void HM_BuzzerStart() { buzzerStart(&buzzer); }

void HM_BuzzerStop() { buzzerStop(&buzzer); }
...
bool HM_UsbIsConnected() { return usbIsConnected(); }

bool HM_UsbTransmit(uint8_t *data, uint16_t numBytes) {
  return usbTransmit(data, numBytes);
}

CircularBuffer_t *HM_UsbGetRxBuffer() { return usbGetRxBuffer(); }
...
void HM_IWDG_Refresh() { HAL_IWDG_Refresh(&hiwdg1); }
