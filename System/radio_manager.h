...
#define RADIO_MAX_STRING 48 // The maximum length one string sent over radio can be. Longer must be segmented
...
PACKED_STRUCT {
	float temp, pos_z, vel_z, lat, lon, gps_alt, batt_volts, speedKnots, courseDeg;
	uint32_t gpsTime;
...
PACKED_STRUCT {
	...
	float altitude, avgAltitude, deltaAltitude, avgDeltaAltitude, temperature, accelX, accelY, accelZ;
} LineCutterPacket_t;
...
void RadioManager_transmitData(SensorData_t* sensorData, FilterData_t* filterData, uint8_t state);
