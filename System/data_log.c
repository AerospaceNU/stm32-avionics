...
void data_log_load_last_stored_flight_metadata() {
  flightNum = data_log_get_last_flight_num();  // Load the previous flight
                                               // number and sector
  uint8_t
      metadataBuff[kFlightMetadataSize];  // Create a buffer for the metadata
...
FlightMetadata data_log_get_metadata() { return metadataPacket; }
...
void data_log_set_launched_metadata() {
  if (flightNum > 0) {
    metadataPacket.launched = 1;  // Indicate that the current flight was
                                  // launched (transitioned past preflight)
  }
}
  ...
  if (flightNum > 0) {
    uint32_t metadataWriteAddress =
        ...
        FLASH_SECTOR_BYTES;  // Metadata is located at the start of the flight
                             // sector
    HM_FlashWriteStart(
        metadataWriteAddress, kFlightMetadataSize,
...
void data_log_get_last_data_packet(uint32_t flightNum, LogData *flightPacket) {
  ...
  uint32_t low = 0;
  uint32_t high =
      (FLASH_SECTOR_BYTES * (lastSector - firstSector + 1)) / kLogDataSize;
  ...
  while (low < mid) {
    ...
    readAddress = firstSector * FLASH_SECTOR_BYTES +
                  FLIGHT_METADATA_PAGES * FLASH_PAGE_SIZE_BYTES +
                  mid * kLogDataSize;
    HM_FlashReadStart(readAddress, kLogDataSize, logPacketBuffer);
    waitStartMS = HM_Millis();
