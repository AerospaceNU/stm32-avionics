...
/**
 * @brief Generate a single doc line by sending to command line with correct formatting
 ...
 */
static void generateDoc(const char* primaryCommand, const char* args, const char* desc) {
	char doc[173 + 5];
...
EndCondition_t CliHelpState::run() {
	...
	cliSend("\r\n");
	generateDoc("--calibrate", "TBD", "Calibration routine is still undefined. Arguments to the command will be guided by that routine");
	generateDoc("--config", "-d float*1", "Configure another drogue cut event's altitude. FCB will re-sort the altitudes automatically");
	...
	generateDoc("--offload", "", "Offloads the last flight recorded on the board");
	...
	generateDoc("--version", "", "Prints git version info for current build");
