...
EndCondition_t CliSenseState::run() {
	...
	cliSend(cliStr);
	sprintf(cliStr, "GPS Time: %02i:%02i:%02i %02i/%02i/%04i\r\n", data->gps_hours, data->gps_minutes, data->gps_seconds, data->gps_month, data->gps_day, data->gps_year);
	cliSend(cliStr);
	sprintf(cliStr, "Battery Voltage (V): %3f\r\n", data->battery_voltage);
	cliSend(cliStr);
	ssize_t numPyros = sizeof(data->pyro_continuity) / sizeof(data->pyro_continuity[0]);
