...
void APP_BLE_Init(void) {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_set_radio_activity_mask command, result: 0x%x \n\r",
        ret);
  } else {
  ...
  //[JT]
  UTIL_SEQ_RegTask(1 << CFG_TASK_LINK_CONFIG_ID, UTIL_SEQ_RFU,
                   LinkConfiguration);
    ...
    case HCI_LE_META_EVT_CODE: {
      ...
      switch (p_meta_evt->subevent) {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE: {
          ...
          APP_DBG_MSG(
              ">>== HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE - Connection "
              "handle: 0x%x\n",
          ...
          HandleNotification.ConnectionHandle =
              BleApplicationContext.BleApplicationContext_legacy
                  .connectionHandle;
          Custom_APP_Notification(&HandleNotification);
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (p_blecore_evt->ecode) {
        ...
        case ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE:
          ...
          if (ret != BLE_STATUS_SUCCESS) {
            APP_DBG_MSG(
                "==>> aci_gap_numeric_comparison_value_confirm_yesno-->YES : "
                "Fail, reason: 0x%x\n",
                ret);
          } else {
            APP_DBG_MSG(
...
static void Ble_Hci_Gap_Gatt_Init(void) {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_write_config_data command - "
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_write_config_data command - CONFIG_DATA_IR_OFFSET, "
        "result: 0x%x \n",
        ret);
  } else {
    APP_DBG_MSG(
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_write_config_data command - CONFIG_DATA_ER_OFFSET, "
        "result: 0x%x \n",
        ret);
  } else {
    APP_DBG_MSG(
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_set_tx_power_level command, result: 0x%x \n", ret);
  } else {
  ...
  if (role > 0) {
    ...
    ret = aci_gap_init(role, CFG_PRIVACY, CFG_GAP_DEVICE_NAME_LENGTH,
                       &gap_service_handle, &gap_dev_name_char_handle,
                       &gap_appearance_char_handle);
    ...
    ret =
        aci_gatt_update_char_value(gap_service_handle, gap_dev_name_char_handle,
                                   0, strlen(name), (uint8_t *)name);
    if (ret != BLE_STATUS_SUCCESS) {
      BLE_DBG_SVCCTL_MSG(
          "  Fail   : aci_gatt_update_char_value - Device Name\n");
    } else {
      BLE_DBG_SVCCTL_MSG(
  ...
  ret =
      aci_gatt_update_char_value(gap_service_handle, gap_appearance_char_handle,
                                 0, 2, (uint8_t *)&a_appearance);
  if (ret != BLE_STATUS_SUCCESS) {
   ...
   */
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
   ...
   */
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .mitm_mode = CFG_MITM_PROTECTION;
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .encryptionKeySizeMin = CFG_ENCRYPTION_KEY_SIZE_MIN;
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .encryptionKeySizeMax = CFG_ENCRYPTION_KEY_SIZE_MAX;
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .Use_Fixed_Pin = CFG_USED_FIXED_PIN;
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .Fixed_Pin = CFG_FIXED_PIN;
  BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
      .bonding_mode = CFG_BONDING_MODE;
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_gap_set_authentication_requirement command, result: "
        "0x%x \n",
        ret);
  } else {
   ...
   */
  if (BleApplicationContext.BleApplicationContext_legacy.bleSecurityParam
          .bonding_mode) {
    ret = aci_gap_configure_whitelist();
    if (ret != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "  Fail   : aci_gap_configure_whitelist command, result: 0x%x \n",
          ret);
    } else {
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus) {
  ...
  ret = aci_gap_set_discoverable(
      ADV_TYPE, CFG_FAST_CONN_ADV_INTERVAL_MIN, CFG_FAST_CONN_ADV_INTERVAL_MAX,
      CFG_BLE_ADDRESS_TYPE, ADV_FILTER, 0, 0, 0, 0, 0, 0);
  if (ret != BLE_STATUS_SUCCESS) {
...
//[JT]
// from
// https://github.com/stm32-hotspot/STM32WB-BLE-Basic-DataThroughput-Server/blob/main/Projects/P-NUCLEO-WB55.Nucleo/Applications/BLE/BLE_Basic_DataThroughput_Server/STM32_WPAN/App/app_ble.c#L862
static void LinkConfiguration(void) {
  ...
  status = hci_le_set_data_length(
      BleApplicationContext.BleApplicationContext_legacy.connectionHandle, 251,
      2120);
  ...
  uint16_t interval_min =
      CONN_P(48.75);  // opimtal for Android Samsung Galaxy S10
  ...
  //  UTIL_SEQ_WaitEvt(
  //      1
  //      << CFG_MY_IDLEEVT_CONN_UPDATE_COMPLETE); /* Wait for Connection Update
  //                                                  complete event to be
  //                                                  recieved before proceeding
  //                                                  */
...
void PHY_Configuration(void) {
  ...
  if (status == BLE_STATUS_SUCCESS) {
    ...
    if ((TX_PHY == TX_2M) && (RX_PHY == RX_2M)) {
      ...
      status = hci_le_set_phy(
          BleApplicationContext.BleApplicationContext_legacy.connectionHandle,
          ALL_PHYS_PREFERENCE, TX_1M, RX_1M, 0);
    } else {
      APP_DBG_MSG("**TX= %d, **RX= %d \n", TX_2M_PREFERRED, RX_2M_PREFERRED);
      status = hci_le_set_phy(
          BleApplicationContext.BleApplicationContext_legacy.connectionHandle,
          ALL_PHYS_PREFERENCE, TX_2M_PREFERRED, RX_2M_PREFERRED, 0);
    }
    if (status == BLE_STATUS_SUCCESS) {
