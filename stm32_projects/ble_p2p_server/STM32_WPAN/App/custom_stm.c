      ...
      switch (blecore_evt->ecode) {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle ==
            ...
            switch (attribute_modified->Attr_Data[0]) {
              ...
              case COMSVC_Notification:
                /* USER CODE BEGIN
                 * CUSTOM_STM_Service_1_Char_2_COMSVC_Notification_BEGIN */
                ...
                Custom_STM_App_Notification(&Notification);
                /* USER CODE BEGIN
                 * CUSTOM_STM_Service_1_Char_2_COMSVC_Notification_END */
            ...
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN
             * CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE
             */
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE:
          ...
          if (read_req->Attribute_Handle ==
            ...
            return_value = SVCCTL_EvtAckFlowEnable;
            /*USER CODE BEGIN
             * CUSTOM_STM_Service_1_Char_2_ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE_1
             */
            ...
            /*USER CODE BEGIN
             * CUSTOM_STM_Service_1_Char_2_ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE_2
             */

            /*USER CODE END
             * CUSTOM_STM_Service_1_Char_2_ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE_2*/
          } /* if (read_req->Attribute_Handle == (CustomContext.CustomTxHdle +
               CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
          ...
          if (write_perm_req->Attribute_Handle ==
              (CustomContext.CustomRxHdle +
               CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* Allow or reject a write request from a client using
             * aci_gatt_write_resp(...) function */
            /*USER CODE BEGIN
...
void SVCCTL_InitCustomSvc(void) {
   ...
   *          bleuart
   *
   * Max_Attribute_Records = 1 + 2*2 +
   * 1*no_of_char_with_notify_or_indicate_property +
   * 1*no_of_char_with_broadcast_property service_max_attribute_record = 1 for
   * bleuart + 2 for bleuart_rxd + 2 for bleuart_txd + 1 for bleuart_txd
   * configuration descriptor + = 6
   *
   * This value doesn't take into account number of descriptors manually added
   * In case of descriptors addded, please update the max_attr_record value
   * accordingly in the next SVCCTL_InitService User Section
   */
  max_attr_record = 6;
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128, (Service_UUID_t *)&uuid,
                             PRIMARY_SERVICE, max_attr_record,
                             &(CustomContext.CustomBlrtHdle));
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_gatt_add_service command: blrt, error code: 0x%x \n\r",
        ret);
  } else {
  ...
  ret = aci_gatt_add_char(CustomContext.CustomBlrtHdle, UUID_TYPE_128, &uuid,
                          SizeRx, CHAR_PROP_WRITE, ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_ATTRIBUTE_WRITE |
                              GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP |
                              GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
                          0x10, CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomRxHdle));
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_gatt_add_char command   : RX, error code: 0x%x \n\r",
        ret);
  } else {
  ...
  ret = aci_gatt_add_char(
      CustomContext.CustomBlrtHdle, UUID_TYPE_128, &uuid, SizeTx,
      CHAR_PROP_READ | CHAR_PROP_NOTIFY, ATTR_PERMISSION_NONE,
      GATT_NOTIFY_ATTRIBUTE_WRITE |
          GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP |
          GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
      0x10, CHAR_VALUE_LEN_VARIABLE, &(CustomContext.CustomTxHdle));
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_gatt_add_char command   : TX, error code: 0x%x \n\r",
        ret);
  } else {
