...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSrvshrtHdle;                    /**< srvlng handle */
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for srvlng +
   *                                2 for My_LED_Char +
   *                              = 3
   *
  ...
  uuid.Char_UUID_16 = 0xfe40;
  ret = aci_gatt_add_service(UUID_TYPE_16,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomSrvshrtHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: srvshrt, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: srvshrt \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomSrvshrtHdle,
                          UUID_TYPE_128, &uuid,
                          SizeLed_C,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomSrvshrtHdle,
                                       CustomContext.CustomLed_CHdle,
                                       0, /* charValOffset */
