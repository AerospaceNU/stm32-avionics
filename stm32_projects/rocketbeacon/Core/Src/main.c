...
void SetModulationParamsFSK(uint32_t bitrate, uint8_t pulseshape,
                            uint8_t bandwidth, uint32_t freq_dev);
void SetPacketParamsLora(uint16_t preamble_length, bool header_fixed,
                         uint8_t payload_length, bool crc_enabled,
                         bool invert_iq);
...
const double LPD433[69] = {
    433.075, 433.100, 433.125, 433.150, 433.175,
    433.200, 433.225, 433.250, 433.275, 433.300,  // 1-10
    433.325, 433.350, 433.375, 433.400, 433.425,
    433.450, 433.475, 433.500, 433.525, 433.550,  // 11-20
    433.575, 433.600, 433.625, 433.650, 433.675,
    433.700, 433.725, 433.750, 433.775, 433.800,  // 21-30
    433.825, 433.850, 433.875, 433.900, 433.925,
    433.950, 433.975, 434.000, 434.025, 434.050,  // 31-40
    434.075, 434.100, 434.125, 434.150, 434.175,
    434.200, 434.225, 434.250, 434.275, 434.300,  // 41-50
    434.325, 434.350, 434.375, 434.400, 434.425,
    434.450, 434.475, 434.500, 434.525, 434.550,  // 51-60
    434.575, 434.600, 434.625, 434.650, 434.675,
    434.700, 434.725, 434.750, 434.775  // 61-69
};
...
const double FRS[22] = {
    ...
    462.6875, 462.7125, 467.5625, 467.5875, 467.6125,  // 1-10
    467.6375, 467.6625, 467.6875, 467.7125, 462.5500,
    462.5750, 462.6000, 462.6250, 462.6500, 462.6750,  // 11-20
...
int main(void)
{
  ...
  // Can be a "standard" frequency, e.g. center_freq = LPD433[20-1]; sets it to
  // LPD433 channel 20 (zero indexed) = 433.550 MHz
  double center_freq = 434.100;

  // max power in dBm, valid values between -9 and 22;
  // If using coin cell batteries, values above 16 dBm are not recommended
  // without testing due to current limitations. Alternatively use external LiPo
  // power
  int maxPower = 16;
  ...
  int CWbeepcount = 4;
  bool CWHigh2Low =
      true;  // if true, start with highest power beep, decrease from there
  ...
  bool FSKbeep = true;
  int FSKbeepcount = 4;
  bool FSKHigh2Low = false;  // if true, start with highest power beep, decrease
                             // from there. When true, it maximizes the power of
                             // the highest power beep, but makes it harder to
                             // read a visible signal indicator on later beeps.
  int FSKbeepIndLength = 150;  // milliseconds
  int FSKbeepGapLength = 50;   // milliseconds
  bool CustomFSKtones = false;
  int CustomFSKfrequencies[] = {320, 400, 480,
                                640};  // Must match the length exactly!

  int Period = 2000;       // milliseconds
  int StartupWait = 5000;  // initial start. A few seconds to allow coin cell
                           // batteries to recover in case of brownout resets.
  ...
  //  int FSKtones[12] = {400, 350, 300, 250, 200, 150, 1600, 2000, 2400, 3200,
  //  4000, 4800};
    ...
    for (int i = 0; i < loopCounter - 1; i++) {
      ...
      if (FSKbeep) {
        if (FSKHigh2Low) {
          ...
          for (int j = 0; j < FSKbeepcount; j++) {
            LED_on();
            FSKBeep(FSKTXpwrs[FSKbeepcount - 1 - j], FSKtones[j],
                    FSKbeepIndLength);
            LED_off();
            HAL_Delay(FSKbeepGapLength);
          }
      ...
      if (CWbeep) {
        if (CWHigh2Low) {
          for (int j = 0; j < CWbeepcount; j++) {
            LED_on();
            SetRfFreq(ComputeRfFreq(
                (center_freq + 0.000001 * CWbeepOffset * j) * freq_correction));
      ...
      /*    	  LED_on();
                ...
                LED_off();
                HAL_Delay(25);
                SetRfFreq(ComputeRfFreq((center_freq + 0.0002) *
         freq_correction)); LED_on(); CWBeep(11, 25); LED_off(); HAL_Delay(25);
                SetRfFreq(ComputeRfFreq((center_freq + 0.0004) *
         freq_correction)); LED_on(); CWBeep(1, 25); LED_off(); HAL_Delay(25);
                SetRfFreq(ComputeRfFreq((center_freq + 0.0006) *
         freq_correction)); LED_on(); CWBeep(-9, 25); LED_off();
...
void SetPacketParamsLora(uint16_t preamble_length, bool header_fixed,
                         uint8_t payload_length, bool crc_enabled,
                         bool invert_iq) {
  uint8_t txbuf[7] = {0x8C,
                      (uint8_t)((preamble_length >> 8) & 0xFF),
                      (uint8_t)(preamble_length & 0xFF),
                      (uint8_t)header_fixed,
                      payload_length,
                      (uint8_t)crc_enabled,
                      (uint8_t)invert_iq};
