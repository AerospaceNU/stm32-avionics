...
class SX126x {
 public:
   ...
   */
  typedef enum { SX1261 = 0, SX1262 = 1, SX1268 = 2 } DeviceType_t;

  /*!
   * \brief Provides the frequency of the chip running on the radio and the
   * frequency step
   *
   * \remark These defines are used for computing the frequency divider to set
   * the RF frequency
   */
  enum {
    XTAL_FREQ = 32000000,
  ...
  enum {
    ...
    /*!
     * \brief Compensation delay for SetAutoTx/Rx functions in 15.625
     * microseconds
     */
    AUTO_RX_TX_OFFSET = 2,
    ...
    /*!
     * The address of the register holding RX Gain value (0x94: power saving,
     * 0x96: rx boosted)
     */
    REG_RX_GAIN = 0x08AC,
 ...
 public:
   ...
   */
  typedef enum {
    RADIO_PACKET_FIXED_LENGTH = 0x00,  //!< The packet is known on both sides,
                                       //!< no header included in the packet
   ...
   */
  typedef struct {
    RadioPacketTypes_t PacketType;  //!< Packet to which the packet parameters
                                    //!< are referring to.
    struct {
      /*!
      ...
      struct {
        ...
        RadioPacketLengthModes_t
            HeaderType;  //!< If the header is explicit, it will be transmitted
                         //!< in the GFSK packet. If the header is implicit, it
      ...
      struct {
        ...
        RadioLoRaPacketLengthsMode_t
            HeaderType;  //!< If the header is explicit, it will be transmitted
                         //!< in the LoRa packet. If the header is implicit, it
                         //!< will not be transmitted
  ...
  typedef enum { PUBLIC = 0, PRIVATE = 1 } NetworkType_t;
  ...
  typedef struct {
    ...
    void (*rxDone)(
        void);  //!< Pointer to a function run on successful reception
    ...
    void (*rxHeaderDone)(
        void);  //!< Pointer to a function run on successful Header reception
    void (*txTimeout)(
        void);  //!< Pointer to a function run on transmission timeout
    void (*rxTimeout)(
        void);  //!< Pointer to a function run on reception timeout
    void (*rxError)(IrqErrorCode_t errCode);  //!< Pointer to a function run on
                                              //!< reception error
    void (*cadDone)(bool cadFlag);  //!< Pointer to a function run on channel
                                    //!< activity detected
  } RadioCallbacks_t;
  ...
  struct {
     ...
     *
     * \param [out] errCode       A code indicating the type of interrupt
     * (SyncWord error or CRC error)
     */
 ...
 public:
   ...
   */
  static uint32_t GetTimeOnAir(const ModulationParams_t &modparams,
                               const PacketParams_t &pktparams);
   ...
   *
   * \param [out] payload       A pointer to a buffer into which the payload
   * will be copied \param [out] size          A pointer to the size of the
   * payload received \param [in]  maxSize       The maximal size allowed to
   ...
   *                              LORA_CAD_16_SYMBOL]
   * \param [in]  cadDetPeak     Limite for detection of SNR peak used in the
   * CAD \param [in]  cadDetMin      Set the minimum symbol recognition for CAD
   ...
   * activity
   */
  void SetCadParams(RadioLoRaCadSymbols_t cadSymbolNum, uint8_t cadDetPeak,
   ...
   */
  virtual void WriteCommand(RadioCommands_t opcode, uint8_t *buffer,
                            uint16_t size);
   ...
   * \param [in]  size          Command parameters byte array size
   */
  virtual void ReadCommand(RadioCommands_t opcode, uint8_t *buffer,
                           uint16_t size);
  ...
  virtual uint8_t HalGpioRead(GpioPinFunction_t func);

  virtual void HalGpioWrite(GpioPinFunction_t func, uint8_t value);

  virtual void HalSpiTransfer(uint8_t *buffer_in, const uint8_t *buffer_out,
                              uint16_t size);
