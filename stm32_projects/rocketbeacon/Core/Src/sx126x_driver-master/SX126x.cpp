...
void SX126x::CheckDeviceReady(void) {
  if ((GetOperatingMode() == MODE_SLEEP) ||
      (GetOperatingMode() == MODE_RX_DC)) {
    Wakeup();
  }
}
...
void SX126x::SetRxBoosted(uint32_t timeout) {
  ...
  WriteReg(REG_RX_GAIN, 0x96);  // max LNA gain, increase current by ~2mA for
...
void SX126x::SetModulationParams(const ModulationParams_t &modulationParams) {
  ...
  switch (modulationParams.PacketType) {
    ...
    case PACKET_TYPE_LORA:
      ...
      if (modulationParams.PacketType == RadioPacketTypes_t::PACKET_TYPE_LORA &&
          modulationParams.Params.LoRa.Bandwidth ==
              RadioLoRaBandwidths_t::LORA_BW_500) {
...
void SX126x::SetPacketParams(const PacketParams_t &packetParams) {
  ...
  switch (packetParams.PacketType) {
    ...
    case PACKET_TYPE_LORA:
      ...
      if (packetParams.Params.LoRa.InvertIQ ==
          RadioLoRaIQModes_t::LORA_IQ_INVERTED) {
...
void SX126x::SetCadParams(RadioLoRaCadSymbols_t cadSymbolNum,
...
void SX126x::SetBufferBaseAddresses(uint8_t txBaseAddress,
...
void SX126x::GetRxBufferStatus(uint8_t *payloadLength,
  ...
  if ((this->GetPacketType() == PACKET_TYPE_LORA) &&
      (ReadReg(REG_LR_PACKETPARAMS) >> 7 == 1)) {
...
void SX126x::ProcessIrqs() {
  ...
  if ((irqRegs & IRQ_HEADER_VALID) == IRQ_HEADER_VALID) {
    // LoRa Only
    //		FrequencyError = 0x000000 | ( ( 0x0F & ReadReg(
    // REG_FREQUENCY_ERRORBASEADDR ) ) << 16 ); 		FrequencyError =
    // FrequencyError | ( ReadReg( REG_FREQUENCY_ERRORBASEADDR + 1 ) << 8 );
    // FrequencyError = FrequencyError | ( ReadReg( REG_FREQUENCY_ERRORBASEADDR
    // +
    // 2 ) );

    if (rxHeaderDone) rxHeaderDone();
  }
  ...
  if ((irqRegs & IRQ_RX_DONE) == IRQ_RX_DONE) {
    if ((irqRegs & IRQ_CRC_ERROR) == IRQ_CRC_ERROR) {
      if (rxError) rxError(IRQ_CRC_ERROR_CODE);
    } else {
      if (rxDone) rxDone();
    }
  }
  ...
  if ((irqRegs & IRQ_SYNCWORD_VALID) == IRQ_SYNCWORD_VALID) {
    if (rxSyncWordDone) rxSyncWordDone();
  }

  if ((irqRegs & IRQ_CAD_DONE) == IRQ_CAD_DONE) {
    if (cadDone) {
      cadDone((irqRegs & IRQ_CAD_ACTIVITY_DETECTED) ==
...
void SX126x::WriteCommand(SX126x::RadioCommands_t opcode, uint8_t *buffer,
                          uint16_t size) {
...
void SX126x::ReadCommand(SX126x::RadioCommands_t opcode, uint8_t *buffer,
                         uint16_t size) {
  WaitOnBusy();
...
void SX126x::WaitOnBusyLong() {
  while (HalGpioRead(GPIO_PIN_BUSY)) {
    //    std::this_thread::sleep_for(std::chrono::microseconds(1)); // todo
  }
}

void SX126x::WaitOnBusy() {
  while (HalGpioRead(GPIO_PIN_BUSY)) {
    //    std::this_thread::sleep_for(std::chrono::microseconds(10)); // todo
  }
}

void SX126x::WaitOnCounter() {
  //  std::this_thread::sleep_for(std::chrono::microseconds(126)); // todo what
  //  the fuck
  HAL_Delay(1);
}

uint32_t SX126x::GetTimeOnAir(const SX126x::ModulationParams_t &modparams,
                              const SX126x::PacketParams_t &pktparams) {
  double ToA = 0;
  ...
  if (modparams.PacketType == PACKET_TYPE_LORA) {
    ...
    uint8_t HeaderSymbols =
        pktparams.Params.LoRa.HeaderType == LORA_PACKET_VARIABLE_LENGTH ? 20
                                                                        : 0;
    ...
    if (SF == 5 || SF == 6) {
      Symbols +=
          6.25 + 8 +
          ceil((fmax(8 * PayloadLength + CrcBits - 4 * SF + HeaderSymbols, 0) /
                (4 * SF))) *
              (CR + 4);
    } else {
      if (modparams.Params.LoRa.LowDatarateOptimize) {
        Symbols +=
            4.25 + 8 +
            ceil((fmax(8 * PayloadLength + CrcBits - 4 * SF + 8 + HeaderSymbols,
                       0) /
                  (4 * (SF - 2)))) *
                (CR + 4);
      } else {
    ...
    BitCount = BitCount + 8 +
               (pktparams.Params.Gfsk.SyncWordLength >> 1) * 8;  // sync word
    BitCount = BitCount + ((pktparams.Params.Gfsk.HeaderType ==
...
// reach deep into the subghz driver to pull out this prototype
extern "C" {
extern HAL_StatusTypeDef SUBGHZ_CheckDeviceReady(SUBGHZ_HandleTypeDef *hsubghz);
extern HAL_StatusTypeDef SUBGHZ_WaitOnBusy(SUBGHZ_HandleTypeDef *hsubghz);
}

// And i want a transcieve function lol
 ...
 * @param  pDataTx the data to transmit
 * @param  pDataRx the SPI data we get back
 * @retval HAL status
 */
static HAL_StatusTypeDef SUBGHZSPI_TransmitRecieve(
    SUBGHZ_HandleTypeDef *hsubghz, const uint8_t *const pDataTx,
    uint8_t *pDataRx) {
  ...
  *spidr = *pDataTx;
  ...
  /* Retrieve pData */
  *pDataRx = (uint8_t)(READ_REG(SUBGHZSPI->DR));
...
void SX126x::HalSpiTransfer(uint8_t *buffer_in, const uint8_t *buffer_out,
  ...
  for (size_t i = 0; i < size; i++)
    SUBGHZSPI_TransmitRecieve(&hsubghz, &buffer_out[i], &buffer_in[i]);
