...
HAL_StatusTypeDef HAL_PCDEx_SetTxFiFoInBytes(PCD_HandleTypeDef *hpcd,
                                             uint8_t fifo, uint16_t size);
HAL_StatusTypeDef HAL_PCDEx_SetRxFiFoInBytes(PCD_HandleTypeDef *hpcd,
                                             uint16_t size);
/* USER CODE END PFP */
...
#if (!STM32F1_DEVICE && defined(PCD_SPEED_HIGH))
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev) {
  ...
  if (pdev->id == DEVICE_HS) {
    ...
    HAL_PCDEx_SetTxFiFoInBytes(hpcd_USB_OTG_PTR, (HID_KEYBOARD_IN_EP & 0x7F),
                               64);
#endif
...
#if (STM32F1_DEVICE)
    /** Device is F1 or similar or if HAL_PCDEx_PMAConfig() is used by HAL
     * driver */
...
#if (USBD_USE_CDC_RNDIS == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_RNDIS_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_RNDIS_OUT_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_RNDIS_CMD_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
#endif
#if (USBD_USE_CDC_ECM == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_ECM_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 48;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_ECM_OUT_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 48;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_ECM_CMD_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
#endif
#if (USBD_USE_HID_MOUSE == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, HID_MOUSE_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
#endif
#if (USBD_USE_HID_KEYBOARD == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, HID_KEYBOARD_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
#endif
#if (USBD_USE_HID_CUSTOM == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CUSTOM_HID_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CUSTOM_HID_OUT_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 8;
#endif
#if (USBD_USE_UAC_MIC == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, AUDIO_MIC_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
#endif
#if (USBD_USE_UAC_SPKR == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, AUDIO_SPKR_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
#endif
#if (USBD_USE_UVC == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, UVC_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
#endif
#if (USBD_USE_MSC == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, MSC_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, MSC_OUT_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
#endif
#if (USBD_USE_DFU == 1)
#endif
#if (USBD_USE_PRNTR == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, PRNT_IN_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, PRNT_OUT_EP,
                        PCD_SNG_BUF, pma_track);
    pma_track += 128;
#endif
#if (USBD_USE_CDC_ACM == 1)
    for (uint8_t i = 0; i < USBD_CDC_ACM_COUNT; i++) {
      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_IN_EP[i],
                          PCD_SNG_BUF, pma_track);
      pma_track += 48;

      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_OUT_EP[i],
                          PCD_SNG_BUF, pma_track);
      pma_track += 48;

      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_CMD_EP[i],
                          PCD_SNG_BUF, pma_track);
      pma_track += 8;
    }
#endif
  ...
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_SOF_CB_ID,
                           PCD_SOFCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_SETUPSTAGE_CB_ID,
                           PCD_SetupStageCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_RESET_CB_ID,
                           PCD_ResetCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_SUSPEND_CB_ID,
                           PCD_SuspendCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_RESUME_CB_ID,
                           PCD_ResumeCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_CONNECT_CB_ID,
                           PCD_ConnectCallback);
  HAL_PCD_RegisterCallback(hpcd_USB_OTG_PTR, HAL_PCD_DISCONNECT_CB_ID,
                           PCD_DisconnectCallback);

  HAL_PCD_RegisterDataOutStageCallback(hpcd_USB_OTG_PTR,
                                       PCD_DataOutStageCallback);
  HAL_PCD_RegisterDataInStageCallback(hpcd_USB_OTG_PTR,
                                      PCD_DataInStageCallback);
  HAL_PCD_RegisterIsoOutIncpltCallback(hpcd_USB_OTG_PTR,
                                       PCD_ISOOUTIncompleteCallback);
  HAL_PCD_RegisterIsoInIncpltCallback(hpcd_USB_OTG_PTR,
