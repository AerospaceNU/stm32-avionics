...
void mcp356x_read(void* pdev) {
  ...
  // Assert CS and start the DMA transaction
  CHIP_SELECT
...
int mcp3564_init(AdcMcp3564Ctrl_s* dev, SPI_HandleTypeDef* hspi,
  ...
  ec = mcp356x_modifyReg8(dev, MCP356X_REG_CONFIG0, 0b1 << 6 | 0b11 << 4 | 0b11,
                          0b0 << 6 | 0b10 << 4 | 0b11);
  if (ec) {
    return ec;
  }
  ...
  // OSR = 128
  //
  ec = mcp356x_modifyReg8(dev, MCP356X_REG_CONFIG1, (1 << 8) - 1,
  ...
  ec = mcp356x_modifyReg8(dev, MCP356X_REG_CONFIG2, (1 << 8) - 1, 0b10001011);
  if (ec) {
    return ec;
  }
  ...
  ec =
      mcp356x_modifyReg8(dev, MCP356X_REG_CONFIG3, 0b11 << 6 | 0b11 << 4 | 0b11,
                         0b11 << 6 | 0b11 << 4 | 0b00);
  if (ec) {
    return ec;
  }
  ...
  ec = mcp356x_modifyReg8(dev, MCP356X_REG_IRQ, (1 << 8) - 1,
                          0b00 << 2 | 0b1 << 1 | 0b0);
  if (ec) {
    return ec;
  }
  ...
  ec = mcp356x_modifyReg24(dev, MCP356X_REG_TIMER, 0xFFFFFF, 0);

  // OFFSETCAL: Force to 0
  ec = mcp356x_modifyReg24(dev, MCP356X_REG_OFFSETCAL, (1 << 24) - 1, 0);
  if (ec) {
    return ec;
  }

  // GAINCAL: Force to unity
  ec = mcp356x_modifyReg24(dev, MCP356X_REG_GAINCAL, (1 << 24) - 1, 8388608);
  if (ec) {
    return ec;
  }

  // Register the external interrupt callback
  // When we get an interrupt here, we will assert the CS pin and start a DMA
  // transaction to read the values from the ADC
  halCallbacks_registerExtInterruptCallback(dev->intPin, mcp356x_read, dev);

  // Register callback for spi transfer complete
  // When the above callback's DMA transaction is completed, this callback is
  // hit which will then deassert CS and process the data read from the ADC
...
int mcp356x_channelSetup(AdcMcp3564Ctrl_s* dev,
  ...
  int ec_write_mux =
      mcp356x_modifyReg8(dev, MCP356X_REG_MUX, (1 << 8) - 1, in_p << 4 | in_n);
  if (ec_write_mux) {
    return ec_write_mux;
  }
