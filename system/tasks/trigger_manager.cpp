...
bool triggerManager_setTriggerConfig(uint8_t triggerNum, const char **configString) {
	...
	ExpressionValueType_e valueType = expressionStore.parseForTrigger(resultID, triggerNum, slice, expressionStore.getNextExpressionSpot(0));
...
void triggerManager_getConfigString(uint8_t triggerNum, char *buffer, int n) {
	expressionStore.conditionToString(triggerNum, buffer, n);
}
void triggerManager_triggerFire(uint8_t triggerNum, bool logFire) {
  ...
  if (triggerConfig->mode == TRIGGER_TYPE_PYRO) {
    ...
    if (!(triggerConfig->port >= 0 && triggerConfig->port < NUM_PYRO)) return;
    hm_pyroSetPwm(triggerConfig->port, triggerConfig->duration, 100, triggerConfig->pulseWidth);
    // Line cutter type
