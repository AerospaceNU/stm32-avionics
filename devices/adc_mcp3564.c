...
static int mcp356x_modify_reg8(AdcMcp3564Ctrl_s *dev, int reg_addr, int mask,
	...
	tx_read[0] = DEV_ADDR << 6 | reg_addr << 2
			| (MCP356X_CMD_TYPE_STATIC_READ & 0x3);

	uint8_t rx_read[2] = { 0 };

	CHIP_SELECT
	int ec_read_reg = HAL_SPI_TransmitReceive(dev->hspi, tx_read, rx_read, 2,
			SPI_TIMEOUT_MS);
	CHIP_DESELCT
	...
	// idx 1 = data
	uint8_t tx_write[2] = { DEV_ADDR << 6 | reg_addr << 2
			| (MCP356X_CMD_TYPE_INCR_WRITE & 0x3), rx_read[1] | val };
...
static int mcp356x_modify_reg24(AdcMcp3564Ctrl_s *dev, int reg_addr, int mask,
	...
	CHIP_SELECT
	int ec_read_reg = HAL_SPI_TransmitReceive(dev->hspi, tx_read, rx_read, 4,
			SPI_TIMEOUT_MS);
	CHIP_DESELCT
	...
	// idx 0 = command byte (Table 6-1)
	// idx 1 = data
	uint8_t tx_write[4] = { DEV_ADDR << 6 | reg_addr << 2
			| (MCP356X_CMD_TYPE_INCR_WRITE & 0x3), reg >> 16, reg >> 8, reg
			& 0xFF };
...
int mcp3564_init(AdcMcp3564Ctrl_s *dev, SPI_HandleTypeDef *hspi,
	...
	uint8_t tx_conv_buf[1] = { DEV_ADDR << 6
			| MCP356X_FAST_CMD_FULL_RESET << 2
	...
	ec = mcp356x_modify_reg8(dev, MCP356X_REG_CONFIG0,
			0b1 << 6 | 0b11 << 4 | 0b11, 0b0 << 6 | 0b10 << 4 | 0b10);
	if (ec) {
		return ec;
	}
	...
	ec = mcp356x_modify_reg8(dev, MCP356X_REG_CONFIG2, (1 << 8) - 1,
			0b10001011);
	if (ec) {
		return ec;
	}
	...
	ec = mcp356x_modify_reg8(dev, MCP356X_REG_CONFIG3,
			0b11 << 6 | 0b11 << 4 | 0b11, 0b10 << 6 | 0b00 << 4 | 0b11);
	if (ec) {
		return ec;
	}
...
int mcp356x_channel_setup(AdcMcp3564Ctrl_s *dev, const int channel_id) {

	// 1: Configure the MUX register to select the correct channel
	// Select MUX_VIN+ as the specified channel and MUX_VOUT- as A_GND
	int ec_write_mux = mcp356x_modify_reg8(dev, MCP356X_REG_MUX, (1 << 8) - 1,
			channel_id << 4 | 0b1100);
	if (ec_write_mux) {
		return ec_write_mux;
	}
	...
	uint8_t tx_conv_buf[1] = { DEV_ADDR << 6
			| MCP356X_FAST_CMD_ADC_CONV_START << 2
			| (MCP356X_CMD_TYPE_FAST_CMD & 0x3), };
...
int mcp356x_read(AdcMcp3564Ctrl_s *dev) {
	...
	CHIP_SELECT
	int ec_read = HAL_SPI_TransmitReceive(dev->hspi, tx_read_buf, rx_read_buf,
			4, SPI_TIMEOUT_MS);
	CHIP_DESELCT
